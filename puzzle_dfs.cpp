...
void dfs(state start, state target){
	int path_cost = 0;
	string out = "";
	map_action path;
	...
	cout << "Expandiendo con DFS" << ENDL;
	
	while (true){
		puzzle = s.top(); s.pop();
		++path_cost;
		
		//cout << puzzle.s << " - - - - - - - - >" << get_state(puzzle.f);
		cout << "Mov: " << puzzle.s << ENDL;
		cout << get_state(puzzle.f) << ENDL;

		// Si ya llegó al estado meta
		if (puzzle.f == target){
			//out += "Solucion Encontrada" + ENDL;
			out += "Nodos expandidos: " + to_string(path_cost) + ENDL; 
			cout << out;
			break;
		}
